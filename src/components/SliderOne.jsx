import React from "react";
import { motion } from "framer-motion";

const SliderOne = () => {
  return (
    <div className="bg-gradient-to-br from-sky-50 to-sky-50 dark:from-slate-900 dark:to-slate-900 py-16 px-4 sm:px-8 lg:px-16">
      <div className="max-w-7xl mx-auto flex flex-col-reverse lg:flex-row items-center gap-10">
        <div className="w-full lg:w-1/2">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 dark:text-white mb-6 leading-tight">
            Organize Your Reading Life with{" "}
            <span className="text-blue-600 dark:text-blue-400">
              Virtual Bookshelf
            </span>
          </h1>

          <p className="text-base text-slate-600 dark:text-slate-300 mb-6">
            A digital platform for book lovers to track, review, and share their
            reading journeys. Keep everything in one place and stay motivated
            with progress insights.
          </p>
        </div>
        <div className="w-full lg:w-1/2 lg:flex justify-center hidden">
          <motion.img
            animate={{ rotate: 360 }}
            transition={{
              ease: "linear",
              duration: 30,
              repeat: Infinity,
            }}
            src="https://cdn-icons-png.flaticon.com/512/3135/3135698.png"
            alt="Bookshelf illustration"
            className="w-80 h-auto drop-shadow-xl dark:invert"
          />
        </div>
      </div>
    </div>
  );
};

export default SliderOne;
